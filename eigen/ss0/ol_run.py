...
if ptb_switch == 1:
    for i in range(0,nz):
        #print(i)
        state = name2var(idx2state[i])
        #print(state.value)
        state.set_value( float(state.value + real_vec[i]) )
        #print(state.value)
...
# update initial conditions to those in steady state solution
for i in m.set_1_NT:
    m.M1set[i].set_value( m_ss.M1[i].value )
    m.M2set[i].set_value( m_ss.M2[i].value )
    for j in m.set_1_2:
        m.x1set[i,j].set_value( m_ss.x1set[i,j].value )
        m.x2set[i,j].set_value( m_ss.x2set[i,j].value )
for f in m.fe:
    m.B1[f].set_value( m_ss.B1.value )
    m.B1[f].fix()
    m.D1[f].set_value( m_ss.D1.value )
    m.D1[f].fix()
    m.VB1[f].set_value( m_ss.VB1.value )
    m.VB1[f].fix()
    m.LT1[f].set_value( m_ss.LT1.value )
    m.LT1[f].fix()
    m.B2[f].set_value( m_ss.B2.value )
    m.B2[f].fix()
    m.D2[f].set_value( m_ss.D2.value )
    m.D2[f].fix()
    m.VB2[f].set_value( m_ss.VB2.value )
    m.VB2[f].fix()
    m.LT2[f].set_value( m_ss.LT2.value )
    m.LT2[f].fix()

# ptb initial conditions along some vector...
# Will have to update variables here "manually"
# To ptb along arbitrary vector from eigen decomp,
# need an nz-dimension vector and some way to interpret it...
# say, [col number]-to->state name dictionary
# sort coords by var they correspond to, 
# maybe with ptb_M1[i], ptb_x1[i,j], etc. data structures... 
...
#p.write('%2i'%'k' + '%8s'%'VB1')
writer = csv.writer(open('ol_data.csv','w',newline=''),delimiter=',')
writer.writerow(['k','cp','LT1','VB1','D1','B1','LT2','VB2','D2','B2','M1_F','M1[1]','M1[NT]','x1[1,1]','x1[1,2]','x1[41,1]','x1[41,2]']) 
...
#            m.zF[0,i].value + m.noise_switch*m.zF_rand[k,i].value)
    ...
    for k in m.fe:
        for c in m.cp:
            writer.writerow([k,c,m.LT1[k].value,m.VB1[k].value,m.D1[k].value,m.B1[k].value,m.LT2[k].value,m.VB2[k].value,m.D2[k].value,m.B2[k].value,m.M1[m.NF,k,c].value,m.M1[1,k,c].value,m.M1[m.NT,k,c].value,m.x1[1,1,k,c].value,m.x1[1,2,k,c].value,m.x1[m.NT,1,k,c].value,m.x1[m.NT,2,k,c].value])
