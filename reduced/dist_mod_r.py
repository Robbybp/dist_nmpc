...
#m.M2_c = Var(m.set_1_NT,m.fe,initialize=0.5)
...
#m.L2_0 = Var(m.set_FT,m.fep)                                       
#m.L2_c = Var(m.set_2_NT,m.fe,initialize=1)
...
#    return m.L1[m.NF,f,c] == m.Kbf*( (m.M1[m.NF,f,c] - m.Muw + sqrt( (m.M1[m.NF,f,c] - m.Muw)**2 + 10**(-8)))/2 )**1.5 # liquid flow below feed
#m.const6 = Constraint(m.fe,m.cp,rule=const6_rule)
...
m.M1dot = Var(m.set_1N,m.fe,m.cp)
...
# L1 and V1 are already defined such that this will be zero
...
# Now calc. x1dot with this constraint
# why are there separate constraints for components 1,2?
def const16_rule(m,f,c):
    return m.M1_0[NF,f]*m.x1dot[NF,1,f,c] == m.L1[NF+1,f]*m.x1[NF+1,1,f,c]-\
        m.L1[NF,f]*m.x1[NF,1,f,c] + \
...
def const17_rule(m,f,c):
    return m.M1_0[NF,f]*m.x1dot[NF,2,f,c] == m.L1[NF+1,f]*m.x1[NF+1,2,f,c]-\
        m.L1[NF,f]*m.x1[NF,2,f,c] + \
...
#def M1_const_rule(m,i,f):
#    return m.M1_0[i,f] == m.M1set[i]
#m.M1_const = Constraint(m.set_2_NTm1,m.fe,rule=M1_const_rule)
...
m.Mx2dot = Var(m.set_1N,m.set_1_2,m.fe,m.cp)
...
#def M2_const_rule(m,i,f):
#    return m.M2_0[i,f] == m.M2set[i] 
#m.M2_const = Constraint(m.set_2_NTm1,m.fe,rule=M2_const_rule)
