...
# set of important (for liquid flow and holdups) trays:
m.set_1FT    = Set(initialize=[1,NF,NT])
m.set_1N     = Set(initialize=[1,NT])
m.set_FT     = Set(initialize=[NF,NT])
...
m.set_NFp1_NT   = Set(initialize=range(NF+1,NT))
m.set_2_NTm1    = Set(initialize=range(2,NT))
m.set_NFp1_NTm2 = Set(initialize=range(NF+1,NT-1))
m.set_dyn       = Set(initialize=[1,NF,NT-1,NT])  # set of dynamic trays
m.set_dyn_L     = Set(initialize=[NF,NT-1,NT])    # set of dynamic liq flow rates
m.set_hyd       = Set(initialize=[NF,NT-1])       # trays at which hydraulics will be considered
...
# *_c variables are "column" variables, not those of the special trays 
#m.M1_c = Var(m.set_1_NT,m.fe,initialize=0.5)
m.V1   = Var(m.set_1_NTm1,m.fe,initialize=1)                             # vapor flow
#m.L1   = Var(m.set_FT,m.fe,m.cp,initialize=1)                          # liquid flow
m.L1   = Var(m.set_2_NT,m.fe,m.cp,initialize=1)                          # liquid flow
m.L1_0 = Var(m.set_2_NT,m.fep)                                       
#m.L1_c = Var(m.set_2_NT,m.fe,initialize=1)
m.D1   = Var(m.fe,initialize=0.57)                                       # distillate flow
m.B1   = Var(m.fe,initialize=0.83)                                       # bottoms flow 
# *_0 variables are just the variables at the 0th collocation point
...
def M1upper_rule(m,i,f):
    return m.M1_0[i,f] <= 0.75 + m.M1eps[i,f]
m.M1upper = Constraint(m.set_2_NTm1,m.fe,rule=M1upper_rule)

def M1_B_upper_rule(m,i,f):
    return m.M1_0[i,f] <= 6.0 + m.M1eps[i,f]
m.M1_B_upper = Constraint([1,NT],m.fe,rule=M1_B_upper_rule)
...
#m.L2   = Var(m.set_FT,m.fe,m.cp,initialize=1)                          # liquid flow
...
### Liquid flows, similarly

# L above feed governed by hydraulics of nearest dynamic tray above
#def rconst1_rule(m,i,f,c):
#    return m.L1[i,f,c] == m.L1[NT-1,f,c] 
#m.rconst1 = Constraint(m.set_NFp1_NTm2,m.fe,m.cp,rule=rconst1_rule)

#def rconst2_rule(m,i,f,c):
#    return m.L1[i,f,c] == m.L1[m.NF,f,c]
#m.rconst2 = Constraint(m.set_2_NFm1,m.fe,m.cp,rule=rconst2_rule)

#def rconst3_rule(m,i,f):
#    return m.M1_c[i,f] == m.M1_0[i,f]
#m.rconst3 = Constraint(m.set_1N,m.fe,rule=rconst3_rule)
...
# ^ only at highest tray in each half-column, see 6 & 7 
...
# L1[NT] indexed by cp even though it won't change that often, for
# consistency with other dynamic L vars 
...
# define M1dot at dynamic trays:
m.M1dot = Var(m.set_dyn,m.fe,m.cp)#,bounds=(-1,1))
...
# Holdup dynamics of tray NT-1
def const12_rule(m,f,c):
    return m.M1dot[NT-1,f,c] == m.L1[NT,f,c] - m.L1[NT-1,f,c] + m.V1[NT-2,f] - m.V1[NT-1,f] # dM above feed
m.const12 = Constraint(m.fe,m.cp,rule=const12_rule)
...
### x1dot calculations replace Mx1dot calculations
...
# Now use component mass balance to calculate x1dot
# No dynamic trays in this range, this is fine as long as M1_0 is const:
# L1 is indexed by c because it is determined by L of a dynamic tray
m.Mx1dot = Var(m.set_dyn,m.set_1_2,m.fe,m.cp)
def const13_rule(m,i,j,f,c):
    return m.M1[i,f,c]*m.x1dot[i,j,f,c] == m.L1[i+1,f,c]*m.x1[i+1,j,f,c] - \
        m.L1[i,f,c]*m.x1[i,j,f,c] + m.V1[i-1,f]*m.y1[i-1,j,f,c] -\
        m.V1[i,f]*m.y1[i,j,f,c] 
m.const13 = Constraint(m.set_2_NFm1,m.set_1_2,m.fe,m.cp,rule=const13_rule)
...
# Component dynamics on tray NT-1, the tray with dynamic holdup:
def const14_dyn_rule(m,j,f,c):
    return m.Mx1dot[NT-1,j,f,c] == m.L1[NT,f,c]*m.x1[NT,j,f,c] - \
        m.L1[NT-1,f,c]*m.x1[NT-1,j,f,c] + m.V1[NT-2,f]*m.y1[NT-2,j,f,c] -\
        m.V1[NT-1,f]*m.y1[NT-1,j,f,c] 
m.const14_dyn = Constraint(m.set_1_2,m.fe,m.cp,rule=const14_dyn_rule)

# Component dynamics on other trays above feed, which have dynamic 
# compositions but not dynamic holdups 
def const14_rule(m,i,j,f,c):
    return m.M1[i,f,c]*m.x1dot[i,j,f,c] == m.L1[i+1,f,c]*m.x1[i+1,j,f,c] - \
        m.L1[i,f,c]*m.x1[i,j,f,c] + m.V1[i-1,f]*m.y1[i-1,j,f,c] -\
        m.V1[i,f]*m.y1[i,j,f,c] 
m.const14 = Constraint(m.set_NFp1_NTm2,m.set_1_2,m.fe,m.cp,rule=const14_rule)
...
# Now calc. Mx1dot at the feed:
# why are there separate constraints for components 1,2?
...
def const18_rule(m,f,c):
    return m.M1dot[1,f,c] == -(19*m.M1dot[NT-1,f,c]+20*m.M1dot[NF,f,c]) - m.V1[1,f] - m.B1[f] 
m.const18 = Constraint(m.fe,m.cp,rule=const18_rule)
...
# changed LT1 in these constraints to L1[NT], for consistency of appearance
def const20_rule(m,f,c):
    return m.M1dot[NT,f,c] == m.V1[NT-1,f] - m.L1[NT,f,c] - m.D1[f] 
m.const20 = Constraint(m.fe,m.cp,rule=const20_rule)

def const20_r_rule(m,c):
    return m.M1dot_r[NT,c] == m.V1[NT-1,0] - m.L1[NT,0,c] - m.D1[0] 
...
# No need for these equations: <- Untrue! Define for trays 1,N
def const21_2_rule(m,i,j,f,c):
    return m.x1dot[i,j,f,c]*m.M1[i,f,c] == (m.Mx1dot[i,j,f,c] - \
        m.x1[i,j,f,c]*m.M1dot[i,f,c])
m.const21_2 = Constraint(m.set_dyn,m.set_1_2,m.fe,m.cp,rule=const21_2_rule)
...
# Make sure M1 simulations are equal to tray above 
# for "non-dynamic" trays
def M1_const_b_rule(m,i,f,c):
    return m.M1[i,f,c] == m.M1[i+1,f,c]
m.M1_b_const = Constraint(m.set_2_NFm1,m.fe,m.cp,rule=M1_const_b_rule)

def M1_const_a_rule(m,i,f,c):
    return m.M1[i,f,c] == m.M1[i+1,f,c]
m.M1_const_a = Constraint(m.set_NFp1_NTm2,m.fe,m.cp,rule=M1_const_a_rule)
...
m.M1_Lag = Constraint(m.set_dyn,m.fe,m.cp,rule=M1_Lag_rule)
...
# Liquid flows are determined by first dynamic tray above:
#def rconst4_rule(m,i,f,c):
#    return m.L2[i,f,c] == m.L2[NT-1,f,c]
#m.rconst4 = Constraint(m.set_NFp1_NTm2,m.fe,m.cp,rule=rconst4_rule)
#
#def rconst5_rule(m,i,f,c):
#    return m.L2[i,f,c] == m.L2[NF,f,c] 
#m.rconst5 = Constraint(m.set_2_NFm1,m.fe,m.cp,rule=rconst5_rule)

#def rconst6_rule(m,i,f):
#    return m.M2_0[i,f] == m.M2_c[i,f]
#m.rconst6 = Constraint(m.set_1N,m.fe,rule=rconst6_rule)

### Liquid flows given by Franci's weir formula: L(i)=K*Mow(i)^1.5
### Liquid flow L(i) only depends on the holdup over the weir Mow(i)
### M(i) = Mow(i) + Muw(i) (Total holdup = holdup over weir + holdup below weir)
# ^ for trays NF and NT-1
...
# L2[NT] defined at cp even though it won't change this often:
...
m.M2dot = Var(m.set_dyn,m.fe,m.cp)# ,bounds=(-1,1))
...
# Mass balance on tray NT-1, which must be considered dynamic
def const33_rule(m,f,c):
    return m.M2dot[NT-1,f,c] == m.L2[NT,f,c] - m.L2[NT-1,f,c] + m.V2[NT-2,f] - m.V2[NT-1,f] # dM above feed
m.const33 = Constraint(m.fe,m.cp,rule=const33_rule)
...
m.Mx2dot = Var(m.set_dyn,m.set_1_2,m.fe,m.cp)
def const34_rule(m,i,j,f,c):
    return m.M2[i,f,c]*m.x2dot[i,j,f,c] == m.L2[i+1,f,c]*m.x2[i+1,j,f,c] - \
...
def const35_rule(m,i,j,f,c):
    return m.M2[i,f,c]*m.x2dot[i,j,f,c] == m.L2[i+1,f,c]*m.x2[i+1,j,f,c] - \
        m.L2[i,f,c]*m.x2[i,j,f,c] + m.V2[i-1,f]*m.y2[i-1,j,f,c]- \
        m.V2[i,f]*m.y2[i,j,f,c] 
m.const35 = Constraint(m.set_NFp1_NTm2,m.set_1_2,m.fe,m.cp,rule=const35_rule)

def const35_dyn_rule(m,j,f,c):
    return m.Mx2dot[NT-1,j,f,c] == m.L2[NT,f,c]*m.x2[NT,j,f,c] - \
        m.L2[NT-1,f,c]*m.x2[NT-1,j,f,c] + m.V2[NT-2,f]*m.y2[NT-2,j,f,c]- \
        m.V2[NT-1,f]*m.y2[NT-1,j,f,c] 
m.const35_dyn = Constraint(m.set_1_2,m.fe,m.cp,rule=const35_dyn_rule)
...
def const39_rule(m,f,c):
    return m.M2dot[1,f,c] == -(19*m.M2dot[NT-1,f,c]+20*m.M2dot[NF,f,c]) - m.V2[1,f] - m.B2[f] 
m.const39 = Constraint(m.fe,m.cp,rule=const39_rule)
...
def const41_rule(m,f,c):
    return m.M2dot[NT,f,c] == m.V2[NT-1,f] - m.L2[NT,f,c] - m.D2[f] 
m.const41 = Constraint(m.fe,m.cp,rule=const41_rule)

def const41_r_rule(m,c):
    return m.M2dot_r[NT,c] == m.V2[NT-1,0] - m.L2[NT,0,c] - m.D2[0] 
...
m.const42_2 = Constraint(m.set_dyn,m.set_1_2,m.fe,m.cp,rule=const42_2_rule)
...
def M2_const_b_rule(m,i,f,c):
    return m.M2[i,f,c] == m.M2[i+1,f,c] 
m.M2_const_b = Constraint(m.set_2_NFm1,m.fe,m.cp,rule=M2_const_b_rule)

def M2_const_a_rule(m,i,f,c):
    return m.M2[i,f,c] == m.M2[i+1,f,c] 
m.M2_const_a = Constraint(m.set_NFp1_NTm2,m.fe,m.cp,rule=M2_const_a_rule)
...
m.M2_Lag = Constraint(m.set_dyn,m.fe,m.cp,rule=M2_Lag_rule)
...
# 1 norm
def descent_con1_rule(m):
    return m.lk == sqrt((m.D1[0]-m.D1ref)**2+m.smt**2) - m.smt + \
        sqrt((m.D2[0]-m.D2ref)**2 + m.smt**2) - m.smt + sqrt((m.B1[0]-m.B1ref)**2 + m.smt**2)- \
m.smt + sqrt((m.B2[0]-m.B1ref)**2+m.smt**2) - m.smt + sqrt((m.VB1[0]-m.VB1ref)**2 + m.smt**2)- \
m.smt + sqrt((m.VB2[0]-m.VB2ref)**2 + m.smt**2) - \
m.smt+ sqrt((m.LT1[0]-m.LT1ref)**2 + m.smt**2) - m.smt + \
       sqrt((m.LT2[0]-m.LT2ref)**2+m.smt**2) - m.smt + \
       sum( sqrt((m.M1_0[i,0]-m.M1ref[i])**2 + m.smt**2) - m.smt + sqrt((m.M2_0[i,0] - m.M2ref[i])**2 + m.smt**2) - m.smt for i in m.set_1N) + \
       sum(sum( sqrt((m.x1_0[i,j,0]-m.x1ref[i,j])**2 + m.smt**2) - m.smt + sqrt((m.x2_0[i,j,0] - m.x2ref[i,j])**2 + m.smt**2) - m.smt for j in m.set_1_2) for i in m.set_1_NT) 
m.descent_con1 = Constraint(rule=descent_con1_rule)

def descent_con2_rule(m):
    return m.vk == sum( sqrt((m.D1[f]-m.D1ref)**2 + m.smt**2) - m.smt + \
        sqrt((m.D2[f]-m.D2ref)**2 +m.smt**2) - m.smt + \
        sqrt((m.B1[f]-m.B1ref)**2 + m.smt**2) - m.smt + \
        sqrt((m.B2[f]-m.B1ref)**2 +m.smt**2) - m.smt + \
        sqrt((m.VB1[f]-m.VB1ref)**2 + m.smt**2) - m.smt + \
        sqrt((m.VB2[f]-m.VB2ref)**2  + m.smt**2) - m.smt + \
        sqrt((m.LT1[f]-m.LT1ref)**2+m.smt**2) - m.smt + \
        sqrt((m.LT2[f]-m.LT2ref)**2+m.smt**2)-m.smt + \
        sum( sqrt((m.M1_0[i,f]-m.M1ref[i])**2+m.smt**2)-m.smt + sqrt((m.M2_0[i,f]-m.M2ref[i])**2+m.smt**2)-m.smt for i in m.set_1N) + \
        sum(sum( sqrt((m.x1_0[i,j,f]-m.x1ref[i,j])**2+m.smt**2)-m.smt + sqrt((m.x2_0[i,j,f]-m.x2ref[i,j])**2 + m.smt**2) - m.smt for j in m.set_1_2) for i in m.set_1_NT ) for f in m.fe) 
...
m.penalty_M_TC = Expression(m.fep,rule=penalty_M_TC_rule)

def penalty_x_y_rule(m,f):
    return m.x2ceps[f] + sum( sum( m.x1eps[i,j,f] + m.x2eps[i,j,f] 
        for j in m.set_1_2) 
        for i in m.set_1_NT) 
m.penalty_x_y = Expression(m.fep,rule=penalty_x_y_rule)
...
def tracking_rule(m,f):
    return (m.VB1[f]-m.VB1ref)**2 + (m.VB2[f]-m.VB2ref)**2 + (m.LT1[f]-m.LT1ref)**2 + \
           ...
           for i in m.set_1_NT) +\
           sum(sum( (m.x1_0[i,j,f]-m.x1ref[i,j])**2 + (m.x2_0[i,j,f]-m.x2ref[i,j])**2   \
               for j in m.set_1_2) \
           for i in m.set_1_NT) 
...
# new: penalty rule defined to include 25th fep when applicable 
def penalty_rule(m):
    return sum(m.penalty_2[f] + m.penalty_V_L[f] for f in m.fe) + \
            sum(m.penalty_M_TC[f] + m.penalty_x_y[f] for f in m.fep)
