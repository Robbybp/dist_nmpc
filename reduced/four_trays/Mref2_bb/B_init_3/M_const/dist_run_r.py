...
for k in m.fep:
    for j in m.set_1_NT:
        ...
        m.M1_0[j,k].set_value( m.M1set[j].value ) #m.M1set[j].value + k/m.nfe*(m.M1ref[j] - m.M1set[j].value) ) 
        m.M2_0[j,k].set_value( m.M2set[j].value ) #m.M2set[j].value + k/m.nfe*(m.M2ref[j] - m.M2set[j].value) ) 
    for j in m.set_dyn:
...
#                m.V2[i-1,f].value - m.V2[i,f].value )
        ...
        m.M1dot[m.NT-1,f,c].set_value( \
            ...
            m.V1[m.NT-2,f].value - m.V1[m.NT-1,f].value ) 
...
#           m.V1[NT-1,f].value - m.LT1[f].value - m.D1[f].value ) 
        ...
        for j in m.set_1_2:
            ...
            for i in m.set_2_NFm1:
                m.x2dot[i,j,f,c].set_value( 1/m.M2[i,f,c].value*(\
                ...
                m.x1dot[i,j,f,c].set_value( 1/m.M1[i,f,c].value*(\
            ...
            m.Mx2dot[m.NT-1,j,f,c].set_value( \
                m.L2[m.NT,f,c].value*m.x2[m.NT,j,f,c].value - \
                m.L2[m.NT-1,f,c].value*m.x2[m.NT-1,j,f,c].value + \
                m.V2[m.NT-2,f].value*m.y2[m.NT-2,j,f,c].value - \
                m.V2[m.NT-1,f].value*m.y2[m.NT-1,j,f,c].value ) 

            m.Mx1dot[m.NT-1,j,f,c].set_value( \
                m.L1[m.NT,f,c].value*m.x1[m.NT,j,f,c].value - \
                m.L1[m.NT-1,f,c].value*m.x1[m.NT-1,j,f,c].value + \
                m.V1[m.NT-2,f].value*m.y1[m.NT-2,j,f,c].value - \
                m.V1[m.NT-1,f].value*m.y1[m.NT-1,j,f,c].value ) 
...
for c in m.cp:
    for i in m.set_1_NTm1:
        for j in m.set_1_2:
            m.y2_r[i,j,c].set_value( m.y2[i,j,0,c].value )
            m.y1_r[i,j,c].set_value( m.y1[i,j,0,c].value ) 
            m.y_1_1_r[i,j,c].set_value( m.y_1_1[i,j,0,c].value )
            m.y_1_2_r[i,j,c].set_value( m.y_1_2[i,j,0,c].value )
            m.y_2_1_r[i,j,c].set_value( m.y_2_1[i,j,0,c].value )
            m.y_2_2_r[i,j,c].set_value( m.y_2_2[i,j,0,c].value )
    for i in m.set_1_NT:
        for j in m.set_1_2:
            m.x1_r[i,j,c].set_value( m.x1[i,j,0,c].value )
            m.x2_r[i,j,c].set_value( m.x2[i,j,0,c].value )
            m.x1dot_r[i,j,c].set_value( m.x1dot[i,j,0,c].value )
            m.x2dot_r[i,j,c].set_value( m.x2dot[i,j,0,c].value ) 
    for i in m.set_dyn:
        m.M1_r[i,c].set_value( m.M1[i,0,c].value )
        m.M2_r[i,c].set_value( m.M2[i,0,c].value )
        m.M1dot_r[i,c].set_value( m.M1dot[i,0,c].value )
        m.M2dot_r[i,c].set_value( m.M2dot[i,0,c].value )
        for j in m.set_1_2:
            m.Mx1dot_r[i,j,c].set_value( m.Mx1dot[i,j,0,c].value )
            m.Mx2dot_r[i,j,c].set_value( m.Mx2dot[i,j,0,c].value )
    for i in m.set_2_NT:
        m.L1_r[i,c].set_value( m.L1[i,0,c].value )
        m.L2_r[i,c].set_value( m.L1[i,0,c].value )

err_x = {}
err_M = {}

writer = csv.writer(open('data.csv','a',newline=''), delimiter=',')
writer.writerow(['k','cp','err_x','err_M','F','M1_F','M1_NTm1','M2_NTm1','LT1','VB1','D1','B1','LT2','VB2','D2','B2'])
        ...
        for c in m.cp:
            for j in m.set_1_2:
                for q in m.fem: 
                    ...
                    m.y_2_2[i,j,q,c].set_value( m.y_2_2[i,j,q+1,c].value ) 
                    # Why is y1 initialized like this?
                    m.y1[i,j,q,c].set_value( m.y1[i,j,q+1,c].value ) 
                    m.y2[i,j,q,c].set_value( m.y2[i,j,q+1,c].value )
            # and y_1_1 etc., for that matter ... 
            m.y_1_1[i,j,m.nfe-1,c].set_value( m.y_1_1ref[i,j] ) 
            m.y_1_2[i,j,m.nfe-1,c].set_value( m.y_1_2ref[i,j] ) 
            m.y_2_1[i,j,m.nfe-1,c].set_value( m.y_2_1ref[i,j] ) 
            m.y_2_2[i,j,m.nfe-1,c].set_value( m.y_2_2ref[i,j] ) 
            m.y1[i,j,m.nfe-1,c].set_value( m.y1ref[i,j] )
            m.y2[i,j,m.nfe-1,c].set_value( m.y2ref[i,j] )
