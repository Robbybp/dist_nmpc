...
def rconst1_rule(m,i,f,c):
    return m.L1[i,f,c] == m.L1[i+1,f,c] 
...
m.M1dot = Var(m.set_dyn,m.fe,m.cp)
...
def const16_rule(m,j,f,c):
    return m.Mx1dot[NF,j,f,c] == m.L1[NF+1,f,c]*m.x1[NF+1,j,f,c]-\
        m.L1[NF,f,c]*m.x1[NF,j,f,c] + \
...
m.M1_const_b = Constraint(m.set_2_NFm1,m.fe,m.cp,rule=M1_const_b_rule)
...
def rconst4_rule(m,i,f,c):
    return m.L2[i,f,c] == m.L2[i+1,f,c]
...
m.M2dot    = Var(m.set_dyn,m.fe,m.cp)
...
def penalty_M_rule(m,f):
    return sum(m.M1eps[i,f] + m.M2eps[i,f] for i in m.set_1_NT)
m.penalty_M = Expression(m.fe,rule=penalty_M_rule)
...
def obj_pen_rule(m):
    return sum(m.rho*m.penalty[f] for f in m.fe)
m.obj = Objective(rule=obj_pen_rule) 
