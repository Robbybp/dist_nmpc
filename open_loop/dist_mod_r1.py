...
m.LT1 = Param(m.fe,mutable=True,initialize=LT1_dict)
VB1_dict = {}
for i in m.fe: VB1_dict[i] = 4.008
m.VB1 = Param(m.fe,mutable=True,initialize=VB1_dict)
...
m.M1   = Var(m.set_1N,m.fe,m.cp,initialize=0.5) # within=NonNegativeReals) # holdup
m.V1   = Var(m.set_1_NTm1,m.fe,initialize=1)                             # vapor flow
#m.L1   = Var(m.set_FT,m.fe,m.cp,initialize=1)                          # liquid flow
m.L1   = Var(m.set_2_NT,m.fe,initialize=1)                          # liquid flow
m.D1   = Param(m.fe,mutable=True,initialize=0.57)                        # distillate flow
m.B1   = Param(m.fe,mutable=True,initialize=0.83)                        # bottoms flow 
...
#m.M1upper = Constraint(m.set_1_NT,m.fe,rule=M1upper_rule)
#
...
m.LT2 = Param(m.fe,mutable=True,initialize=2.13827)
m.VB2 = Param(m.fe,mutable=True,initialize=2.40367)
...
m.M2   = Var(m.set_1N,m.fe,m.cp,initialize=0.5)                        # holdup
m.V2   = Var(m.set_1_NTm1,m.fe,initialize=1)                             # vapor flow
m.L2   = Var(m.set_2_NT,m.fe,initialize=1)                          # liquid flow
m.D2   = Param(m.fe,mutable=True,initialize=0.26)                        # distillate flow
m.B2   = Param(m.fe,mutable=True,initialize=0.56)                        # bottoms flow 
...
def rconst1_rule(m,i,f):
    return m.L1[i,f] == m.LT1[f] 
m.rconst1 = Constraint(m.set_NFp1_NT,m.fe,rule=rconst1_rule)

def rconst2_rule(m,i,f):
    return m.L1[i,f] == m.LT1[f] + m.qF[f]*m.F[f]
m.rconst2 = Constraint(m.set_2_NF,m.fe,rule=rconst2_rule)
...
def const8_rule(m,f):
    return m.L1[NT,f] == m.LT1[f]  # condenser's liquid flow
m.const8 = Constraint(m.fe,rule=const8_rule)
...
m.Mx1dot = Var(m.set_1N,m.set_1_2,m.fe,m.cp)

def const13_rule(m,i,j,f,c):
    return m.M1_0[i,f]*m.x1dot[i,j,f,c] == m.L1[i+1,f]*m.x1[i+1,j,f,c] - \
        m.L1[i,f]*m.x1[i,j,f,c] + m.V1[i-1,f]*m.y1[i-1,j,f,c] -\
        m.V1[i,f]*m.y1[i,j,f,c] 
m.const13 = Constraint(m.set_2_NFm1,m.set_1_2,m.fe,m.cp,rule=const13_rule)

# calculate x1dot for first reduced model:
...
def const16_rule(m,j,f,c):
    return m.M1_0[NF,f]*m.x1dot[NF,j,f,c] == m.L1[NF+1,f]*m.x1[NF+1,j,f,c]-\
        m.L1[NF,f]*m.x1[NF,j,f,c] + \
        m.V1[NF-1,f]*m.y1[NF-1,j,f,c]-m.V1[NF,f]*m.y1[NF,j,f,c] +\
        m.F[f]*m.zF[f,j] 
m.const16 = Constraint(m.set_1_2,m.fe,m.cp,rule=const16_rule)
...
### (which is not relevant for the following calculations)

def const18_rule(m,f,c):
    return m.M1dot[1,f,c] == m.L1[2,f] - m.V1[1,f] - m.B1[f] 
m.const18 = Constraint(m.fe,m.cp,rule=const18_rule)

def const19_rule(m,j,f,c):
    return m.Mx1dot[1,j,f,c] == m.L1[2,f]*m.x1[2,j,f,c] - \
...
def const20_rule(m,f,c):
    return m.M1dot[NT,f,c] == m.V1[NT-1,f] - m.L1[NT,f] - m.D1[f] 
...
def const21_rule(m,j,f,c):
    return m.Mx1dot[NT,j,f,c] == m.V1[NT-1,f]*m.y1[NT-1,j,f,c] -\
        m.L1[NT,f]*m.x1[NT,j,f,c] - m.D1[f]*m.x1[NT,j,f,c] 
...
m.const21_2 = Constraint(m.set_1N,m.set_1_2,m.fe,m.cp,rule=const21_2_rule)
...
# Make sure M1 simulations are constant at IC's for the 
# non-dynamic trays

def M1_const_a_rule(m,i,f):
    return m.M1_0[i,f] == m.M1set[i]
m.M1_const_a = Constraint(m.set_2_NTm1,m.set_1_nfe,rule=M1_const_a_rule)

#def M1_const_b_rule(m,i,f):
#    return m.M1_0[i,f] == m.M1set[i]
#m.M1_const_b = Constraint(m.set_2_N

def M1_constr_rule(m,i,f):
    return m.M1_0[i,f] == m.M1[i,f-1,m.ncp]
m.M1_constr = Constraint(m.set_1N,m.set_1_nfe,rule=M1_constr_rule)

def M1_Lag_rule(m,i,q,c):
    return m.M1[i,q,c] == m.M1_0[i,q] + m.h[q]*sum(m.omega[k,c]*m.M1dot[i,q,k] for k in m.cp)
m.M1_Lag = Constraint(m.set_1N,m.fe,m.cp,rule=M1_Lag_rule)
...
def rconst4_rule(m,i,f):
    return m.L2[i,f] == m.LT2[f]
m.rconst4 = Constraint(m.set_NFp1_NT,m.fe,rule=rconst4_rule)

def rconst5_rule(m,i,f):
    return m.L2[i,f] == m.LT2[f] + m.B1[f] 
m.rconst5 = Constraint(m.set_2_NF,m.fe,rule=rconst5_rule)
...
def const29_rule(m,f):
    return m.L2[NT,f] == m.LT2[f]  # condenser's liquid flow
m.const29 = Constraint(m.fe,rule=const29_rule)

### Material balances for total holdup and component holdup ###

m.x2dot      = Var(m.set_1_NT,m.set_1_2,m.fe,m.cp)

m.M2dot    = Var(m.set_1N,m.fe,m.cp)
...
def const34_rule(m,i,j,f,c):
    return m.M2_0[i,f]*m.x2dot[i,j,f,c] == m.L2[i+1,f]*m.x2[i+1,j,f,c] - \
        m.L2[i,f]*m.x2[i,j,f,c] + m.V2[i-1,f]*m.y2[i-1,j,f,c]- \
...
def const37_rule(m,f,j,c):
    return m.M2_0[NF,f]*m.x2dot[NF,j,f,c] == m.L2[NF+1,f]*m.x2[NF+1,j,f,c]-\
        m.L2[NF,f]*m.x2[NF,j,f,c] + \
...
def const39_rule(m,f,c):
    return m.M2dot[1,f,c] == m.L2[2,f] - m.V2[1,f] - m.B2[f] 
m.const39 = Constraint(m.fe,m.cp,rule=const39_rule)

def const40_rule(m,j,f,c):
    return m.Mx2dot[1,j,f,c] == m.L2[2,f]*m.x2[2,j,f,c] - m.V2[1,f]*m.y2[1,j,f,c] - m.B2[f]*m.x2[1,j,f,c]
m.const40 = Constraint(m.set_1_2,m.fe,m.cp,rule=const40_rule)

### Total Condenser (not equilibrium)

def const41_rule(m,f,c):
    return m.M2dot[NT,f,c] == m.V2[NT-1,f] - m.L2[NT,f] - m.D2[f] 
m.const41 = Constraint(m.fe,m.cp,rule=const41_rule)

def const42_rule(m,j,f,c):
    return m.Mx2dot[NT,j,f,c] == m.V2[NT-1,f]*m.y2[NT-1,j,f,c] - m.L2[NT,f]*m.x2[NT,j,f,c] - m.D2[f]*m.x2[NT,j,f,c] 
...
m.const42_2 = Constraint(m.set_1N,m.set_1_2,m.fe,m.cp,rule=const42_2_rule)
...
def M2_const_a_rule(m,i,f):
    return m.M2_0[i,f] == m.M2set[i]
m.M2_const_a = Constraint(m.set_2_NTm1,m.set_1_nfe,rule=M2_const_a_rule)

def M2_constr_rule(m,i,f):
    return m.M2_0[i,f] == m.M2[i,f-1,m.ncp]
m.M2_constr = Constraint(m.set_1N,m.set_1_nfe,rule=M2_constr_rule)

def M2_Lag_rule(m,i,q,c):
    return m.M2[i,q,c] == m.M2_0[i,q] + m.h[q]*sum(m.omega[k,c]*m.M2dot[i,q,k] for k in m.cp)
m.M2_Lag = Constraint(m.set_1N,m.fe,m.cp,rule=M2_Lag_rule)

# Constraints not in original model: 
# just going to ignore TC constraints for reduced model...
