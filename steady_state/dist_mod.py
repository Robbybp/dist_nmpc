...
m.numstat     = Param(initialize=6*m.NT) # differential states  
m.state_set   = Set(initialize=range(1,m.numstat+1))
m.state_act   = Param(m.state_set,mutable=True)   
#m.state_set.pprint()

# what is sol_stat? 
# solver status 
m.sol_stat    = Param(mutable=True) 
#m.sol_stat    = {} 
m.step_cost   = Param(initialize=0,mutable=True)        

# calculated a posteriori ?
#m.avg_cost    = Param(mutable=True)          
m.vkmin1      = Param(mutable=True)        
m.lkmin1      = Param(mutable=True)       
m.vtr         = Param(mutable=True)
m.termepsk    = Param(initialize=0,mutable=True)        
m.descepsk    = Param(initialize=0,mutable=True)      
m.ltr         = Param(mutable=True) 
m.normx       = Param(mutable=True)         
m.sigcosttest = Param(m.set_1_2,range(1,8))
# ^ removed indexing by k
...
# Disturbances...
# removed indexing by fe:
m.F           = Param(initialize=1.41,mutable=True)           
m.qF          = Param(initialize=1.00,mutable=True)          
m.pV          = Param(initialize=8.0e-3,mutable=True)      
def zF_init(m,j):
    return nominal_zF_dict[j]
m.zF          = Param(m.comp,initialize=zF_init,mutable=True)
...
#m.F_rand      = Param(mutable=True)                 
#m.zF_rand     = Param(m.comp,mutable=True)                      
...
## FIRST COLUMN ##
##################
m.LT1 = Var(within=NonNegativeReals, bounds=(0,10), initialize=3.43656)
m.VB1 = Var(within=NonNegativeReals, bounds=(0,10), initialize=4.008)

# component fraction liquid and vapor
m.y1   = Var(m.set_1_NTm1,m.set_1_2,initialize=0.3) # vapor frac
m.x1   = Var(m.set_1_NT,  m.set_1_2,initialize=0.4) # liquid frac
m.M1   = Var(m.set_1_NT,initialize=0.5) # within=NonNegativeReals)                       # holdup
m.V1   = Var(m.set_1_NTm1,initialize=1)                             # vapor flow
m.L1   = Var(m.set_2_NT,initialize=1)                          # liquid flow
m.D1   = Var(initialize=0.57)                                       # distillate flow
m.B1   = Var(initialize=0.83)                                       # bottoms flow 
...
m.B1eps  = Var(within=NonNegativeReals,initialize=0) 
m.TC1eps = Var(m.set_1_NT,within=NonNegativeReals,initialize=0)

# soft constraints:
# (soft constraints determine the *eps variables, which are penalized) 

# does this x1_0 constraint do anything????
def x1lower_rule(m,i,j):
    return m.x1[i,j] >= -m.x1eps[i,j] 
...
m.V1lower = Constraint(m.set_1_NTm1,rule=V1lower_rule)

def L1lower_rule(m,i):
    return m.L1[i] >= -m.L1eps[i]
m.L1lower = Constraint(m.set_2_NT,rule=L1lower_rule)

def D1lower_rule(m):
    return m.D1 >= -m.D1eps
m.D1lower = Constraint(rule=D1lower_rule)

def B1lower_rule(m):
    return m.B1 >= -m.B1eps 
m.B1lower = Constraint(rule=B1lower_rule)

# VLE equation split
m.y_1_1 = Var(m.set_1_NTm1,m.set_1_2,initialize=0.2)
m.y_1_2 = Var(m.set_1_NTm1,m.set_1_2,initialize=0.7)
...
#
#def y1_0_rule(m,i,j):
#    return m.y_1_1_0[i,j] / m.y_1_2_0[i,j] 
#m.y1_0 = Expression(m.set_1_NTm1,m.set_1_2,rule=y1_0_rule)

# TC1, SET1, and SET2 are not as in the original model (idk what this means just copying Devin)
m.TC1 = Var(m.set_1_NT,initialize=380)
...
m.LT2 = Var(bounds=(0,10), initialize=2.13827)
m.VB2 = Var(bounds=(0,10), initialize=2.40367)

# component fraction liquid and vapor
m.y2   = Var(m.set_1_NTm1,m.set_1_2,initialize=0.5) # vapor frac
m.x2   = Var(m.set_1_NT,  m.set_1_2,initialize=0.4) # liquid frac
m.M2   = Var(m.set_1_NT,initialize=0.5)                        # holdup
m.V2   = Var(m.set_1_NTm1,initialize=1)                             # vapor flow
m.L2   = Var(m.set_2_NT,initialize=1)                          # liquid flow
m.D2   = Var(initialize=0.26)                                       # distillate flow
m.B2   = Var(initialize=0.56)                                       # bottoms flow 
...
m.B2eps   = Var(within=NonNegativeReals,initialize=0) 
m.TC2eps  = Var(m.set_1_NT,within=NonNegativeReals,initialize=0)
m.termeps = Var(within=NonNegativeReals,initialize=0)
m.desceps = Var(within=NonNegativeReals,initialize=0)

# soft constraints:

def x2lower_rule(m,i,j):
    return m.x2[i,j] >= -m.x2eps[i,j] 
...
m.V2lower = Constraint(m.set_1_NTm1,rule=V2lower_rule)

def L2lower_rule(m,i):
    return m.L2[i] >= -m.L2eps[i]
m.L2lower = Constraint(m.set_2_NT,rule=L2lower_rule)

def D2lower_rule(m):
    return m.D2 >= -m.D2eps
m.D2lower = Constraint(rule=D2lower_rule)

def B2lower_rule(m):
    return m.B2 >= -m.B2eps 
m.B2lower = Constraint(rule=B2lower_rule)

def const45_rule(m):
    return m.x1[NT,1] >= 0.95-m.x1eps[NT,1]                  # xA
m.const45 = Constraint(rule=const45_rule)

def const46_rule(m):
    return m.x2[NT,2] >= 0.95-m.x2eps[NT,2]                  # xB
m.const46 = Constraint(rule=const46_rule)

def const47_rule(m):
    return 1 - (m.x2[1,1]+m.x2[1,2]) >= 0.95 - m.x2ceps # xC, I can only assume
m.const47 = Constraint(rule=const47_rule)

# VLE equation split
m.y_2_1 = Var(m.set_1_NTm1,m.set_1_2,initialize=0.3)
m.y_2_2 = Var(m.set_1_NTm1,m.set_1_2,initialize=0.8)
# These variables are "defined" (per ampl) according to others
# I think this funcionality is best reproduced by pyomo expressions
#def y_2_1_0_rule(m,i,j):
#    return m.x2[i,j]*m.alpha_m[j,j] 
#m.y_2_1_0 = Expression(m.set_1_NTm1,m.set_1_2,rule=y_2_1_0_rule)
#
#def y_2_2_0_rule(m,i,j):
#    return ((m.x2_0[i,1]*(m.alpha[1]-1)+m.x2_0[i,2]*(m.alpha[2]-1))+1)
#m.y_2_2_0 = Expression(m.set_1_NTm1,m.set_1_2,rule=y_2_2_0_rule)
#
#def y2_0_rule(m,i,j):
#    return m.y_2_1_0[i,j] / m.y_2_2_0[i,j] 
#m.y2_0 = Expression(m.set_1_NTm1,m.set_1_2,rule=y2_0_rule)  # vapor comp
...
def const1_rule(m,i,j):
    return m.y_1_1[i,j] == m.x1[i,j]*m.alpha_m[j,j]
m.const1 = Constraint(m.set_1_NTm1,m.set_1_2,rule=const1_rule)

def const2_rule(m,i,j):
    return m.y_1_2[i,j] == ((m.x1[i,1]*(m.alpha[1]-1)+m.x1[i,2]*(m.alpha[2]-1))+1)
m.const2 = Constraint(m.set_1_NTm1,m.set_1_2,rule=const2_rule)

# Vapor-liquid equilibria (multicomponent ideal VLE
# Stichlmair-Fair, 'Distillation', p.36, 1998)

def const3_rule(m,i,j):
    return m.y1[i,j]*m.y_1_2[i,j] == m.y_1_1[i,j]
m.const3 = Constraint(m.set_1_NTm1,m.set_1_2,rule=const3_rule)

### Vapor flows assuming constant molar flows ###

def const4_rule(m,i):
    return m.V1[i] == m.VB1                            # vapor flow below feed
m.const4 = Constraint(m.set_1_NFm1,rule=const4_rule)

def const5_rule(m,i):
    return m.V1[i] == m.VB1 + (1-m.qF)*m.F      # vapor flow above feed 
m.const5 = Constraint(m.set_NF_NTm1,rule=const5_rule) 
...
def const6_rule(m,i):
    return m.L1[i] == m.Kbf*( (m.M1[i] - m.Muw + sqrt( (m.M1[i] - m.Muw)**2 + 10**(-8)))/2 )**1.5 # liquid flow below feed
m.const6 = Constraint(m.set_2_NF,rule=const6_rule)

def const7_rule(m,i):
    return m.L1[i] == m.Kuf*( (m.M1[i] - m.Muw + sqrt( (m.M1[i] - m.Muw)**2 + 10**(-8)))/2 )**1.5 # liquid flows above feed 
m.const7 = Constraint(m.set_NFp1_NTm1,rule=const7_rule)

def const8_rule(m):
    return m.L1[NT] == m.LT1  # condenser's liquid flow
m.const8 = Constraint(rule=const8_rule)

### Material balances for total holdup and component holdup ###
#m.M1dot = Var(m.set_1_NT)
def const11_rule(m,i):
    return 0 == m.L1[i+1] - m.L1[i] + m.V1[i-1] - m.V1[i] # dM below feed
m.const11 = Constraint(m.set_2_NFm1,rule=const11_rule)

def const12_rule(m,i):
    return 0 == m.L1[i+1] - m.L1[i] + m.V1[i-1] - m.V1[i] # dM above feed
m.const12 = Constraint(m.set_NFp1_NTm1,rule=const12_rule)

#m.Mx1dot = Var(m.set_1_NT,m.set_1_2,m.fe,m.cp)
def const13_rule(m,i,j):
    return 0 == m.L1[i+1]*m.x1[i+1,j] - \
        m.L1[i]*m.x1[i,j] + m.V1[i-1]*m.y1[i-1,j] -\
        m.V1[i]*m.y1[i,j] 
m.const13 = Constraint(m.set_2_NFm1,m.set_1_2,rule=const13_rule)

def const14_rule(m,i,j):
    return 0 == m.L1[i+1]*m.x1[i+1,j] - \
        m.L1[i]*m.x1[i,j] + m.V1[i-1]*m.y1[i-1,j] -\
        m.V1[i]*m.y1[i,j] 
...
def const18_rule(m):
    return 0 == m.L1[2] - m.V1[1] - m.B1 
m.const18 = Constraint(rule=const18_rule)

def const19_rule(m,j):
    return 0 == m.L1[2]*m.x1[2,j] - \
        m.V1[1]*m.y1[1,j] - m.B1*m.x1[1,j]
m.const19 = Constraint(m.set_1_2,rule=const19_rule)

### Total Condenser (not equilibrium)

def const20_rule(m):
    return 0 == m.V1[NT-1] - m.LT1 - m.D1 
m.const20 = Constraint(rule=const20_rule)

def const21_rule(m,j):
    return 0 == m.V1[NT-1]*m.y1[NT-1,j] -\
        m.L1[NT]*m.x1[NT,j] - m.D1*m.x1[NT,j] 
m.const21 = Constraint(m.set_1_2,rule=const21_rule) 
...
def const22_rule(m,i,j):
    return m.y_2_1[i,j] == m.x2[i,j]*m.alpha_m[j,j]
m.const22 = Constraint(m.set_1_NTm1,m.set_1_2,rule=const22_rule)

def const23_rule(m,i,j):
    return m.y_2_2[i,j] == ((m.x2[i,1]*(m.alpha[1]-1)+m.x2[i,2]*(m.alpha[2]-1))+1)
m.const23 = Constraint(m.set_1_NTm1,m.set_1_2,rule=const23_rule)

# Vapor-liquid equilibria (multicomponent ideal VLE
# Stichlmair-Fair, 'Distillation', p.36, 1998)

def const24_rule(m,i,j):
    return m.y2[i,j]*m.y_2_2[i,j] == m.y_2_1[i,j]
m.const24 = Constraint(m.set_1_NTm1,m.set_1_2,rule=const24_rule)

### Vapor flows assuming constant molar flows ###

def const25_rule(m,i):
    return m.V2[i] == m.VB2                            # vapor flow below feed
m.const25 = Constraint(m.set_1_NFm1,rule=const25_rule)

def const26_rule(m,i):
    return m.V2[i] == m.VB2                            # vapor flow above feed 
m.const26 = Constraint(m.set_NF_NTm1,rule=const26_rule) 

### Liquid flows given by Franci's weir formula: L(i)=K*Mow(i)^1.5
### Liquid flow L(i) only depends on the holdup over the weir Mow(i)
### M(i) = Mow(i) + Muw(i) (Total holdup = holdup over weir + holdup below weir)

def const27_rule(m,i):
    return m.L2[i] == m.Kbf*( (m.M2[i] - m.Muw + sqrt( (m.M2[i] - m.Muw)**2 + 10**(-8)))/2 )**1.5 # liquid flow below feed
m.const27 = Constraint(m.set_2_NF,rule=const27_rule)

def const28_rule(m,i):
    return m.L2[i] == m.Kuf*( (m.M2[i] - m.Muw + sqrt( (m.M2[i] - m.Muw)**2 + 10**(-8)))/2 )**1.5 # liquid flows above feed 
m.const28 = Constraint(m.set_NFp1_NTm1,rule=const28_rule)

def const29_rule(m):
    return m.L2[NT] == m.LT2  # condenser's liquid flow
m.const29 = Constraint(rule=const29_rule)

### Material balances for total holdup and component holdup ###
#m.M2dot = Var(m.set_1_NT,m.fe,m.cp)
def const32_rule(m,i):
    return 0 == m.L2[i+1] - m.L2[i] + m.V2[i-1] - m.V2[i] # dM below feed
m.const32 = Constraint(m.set_2_NFm1,rule=const32_rule)

def const33_rule(m,i):
    return 0 == m.L2[i+1] - m.L2[i] + m.V2[i-1] - m.V2[i] # dM above feed
m.const33 = Constraint(m.set_NFp1_NTm1,rule=const33_rule)

#m.Mx2dot = Var(m.set_1_NT,m.set_1_2,m.fe,m.cp)
def const34_rule(m,i,j):
    return 0 == m.L2[i+1]*m.x2[i+1,j] - \
        m.L2[i]*m.x2[i,j] + m.V2[i-1]*m.y2[i-1,j]- \
        m.V2[i]*m.y2[i,j] 
m.const34 = Constraint(m.set_2_NFm1,m.set_1_2,rule=const34_rule)

def const35_rule(m,i,j):
    return 0 == m.L2[i+1]*m.x2[i+1,j] - \
        m.L2[i]*m.x2[i,j] + m.V2[i-1]*m.y2[i-1,j]- \
        m.V2[i]*m.y2[i,j] 
m.const35 = Constraint(m.set_NFp1_NTm1,m.set_1_2,rule=const35_rule)

### Correction at feed stage. Feed assumed to be (well?) mixed into feed stage.

def const36_rule(m):
    return 0 == m.L2[NF+1] - m.L2[NF] + m.V2[NF-1] - m.V2[NF] + m.B1
m.const36 = Constraint(rule=const36_rule)

def const37_rule(m,j):
    return 0 == m.L2[NF+1]*m.x2[NF+1,j]-\
        m.L2[NF]*m.x2[NF,j] + \
        m.V2[NF-1]*m.y2[NF-1,j]- m.V2[NF]*m.y2[NF,j]+\
        m.B1*m.x1[1,j]  
m.const37 = Constraint(m.set_1_2,rule=const37_rule)

### Reboiler, assumed to be an equilibrium stage 

def const39_rule(m):
    return 0 == m.L2[2] - m.V2[1] - m.B2 
m.const39 = Constraint(rule=const39_rule)

def const40_rule(m,j):
    return 0 == m.L2[2]*m.x2[2,j] - m.V2[1]*m.y2[1,j] - m.B2*m.x2[1,j]
m.const40 = Constraint(m.set_1_2,rule=const40_rule)

### Total Condenser (not equilibrium)

def const41_rule(m):
    return 0 == m.V2[NT-1] - m.LT2 - m.D2 
m.const41 = Constraint(rule=const41_rule)

def const42_rule(m,j):
    return 0 == m.V2[NT-1]*m.y2[NT-1,j] - m.L2[NT]*m.x2[NT,j] - m.D2*m.x2[NT,j] 
m.const42 = Constraint(m.set_1_2,rule=const42_rule) 
...
def const43_rule(m,i):
    return m.TC1[i] == m.x1[i,1]*353.3 + m.x1[i,2]*383.8 + (1-m.x1[i,1]-m.x1[i,2])*411.5
m.const43 = Constraint(m.set_1_NT,rule=const43_rule)

def const44_rule(m,i):
    return m.TC2[i] == m.x2[i,1]*353.3 + m.x2[i,2]*383.8 + (1-m.x2[i,1]-m.x2[i,2])*411.5 #m.x1[1,4,1]*353.3+m.x1[14,2]*383.8 + (1-m.x1[14,2])*411.5 == 382.9450 ???
m.const44 = Constraint(m.set_1_NT,rule=const44_rule)
...
#def termcon_rule(m):
#    return sum( sum( ((m.x1[i,j]-m.x1ref[i,j])**2 + \
...
def cost_rule(m):
    return m.pF*m.F + m.pV*(m.VB1 + m.VB2) - m.pA*m.D1 - m.pB*m.D2 - m.pC*m.B2 
m.cost = Expression(rule=cost_rule)

def penalty_2_rule(m):
    return m.D1eps + m.B1eps + m.D2eps + m.B2eps 
...
def penalty_x_y_rule(m):
    return m.x2ceps + sum( sum( m.x1eps[i,j] + m.x2eps[i,j] 
        for j in m.set_1_2) 
        for i in m.set_1_NT) 
m.penalty_x_y = Expression(rule=penalty_x_y_rule)

def penalty_V_L_rule(m):
    return sum( m.V1eps[i] + m.V2eps[i] for i in m.set_1_NTm1) +\
            sum(m.L1eps[i]+m.L2eps[i] for i in m.set_2_NT)
m.penalty_V_L = Expression(rule=penalty_V_L_rule)


def tracking_rule(m):
    return (m.VB1-m.VB1ref)**2 + (m.VB2-m.VB2ref)**2 + (m.LT1-m.LT1ref)**2 + \
           (m.LT2-m.LT2ref)**2 + (m.D1 - m.D1ref)**2 + (m.B1 - m.B1ref)**2 + \
           (m.D2 - m.D2ref)**2 + (m.B2 - m.B2ref)**2 + \
           sum(     (m.M1[i]  -  m.M1ref[i])**2 + (m.M2[i]  -  m.M2ref[i])**2 + \
               sum( (m.x1[i,j]-m.x1ref[i,j])**2 + (m.x2[i,j]-m.x2ref[i,j])**2   \
               for j in m.set_1_2) \
           for i in m.set_1_NT) 
m.tracking = Expression(rule=tracking_rule) 
...
def penalty_rule(m):
    return m.penalty_2 + m.penalty_M_TC + \
            m.penalty_x_y + m.penalty_V_L 
m.penalty = Expression(rule=penalty_rule)
...
def objtr_rule(m):
    return m.rho*m.pen_switch*(m.termeps+m.desceps) + \
        m.rho*m.pen_switch*m.penalty + \
        m.track_switch*m.tracking 
#m.obj = Objective(rule=objtr_rule) 

def objss_rule(m):
    return m.rho*m.pen_switch*m.penalty 
m.obj = Objective(rule=objss_rule)
